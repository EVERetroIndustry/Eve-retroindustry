...
- **What a contract is worth** - expand any contract's items and each line is valued, with the total under it and the difference against what the contract asks. Prices come from your own data, best source first: **Jita sell**, then the cheapest **single-item contract** indexed anywhere (a titan is never on a Jita sell order - it is on a contract), then CCP's adjusted price marked as an estimate; anything with no price at all is counted and named, so the total never quietly omits half the pile. Contracts in systems held by a player alliance are not used as references - in sov space the market is not open, while NPC null is fine. One click copies the item list for anywhere else
